...
- **Decision:**
  ...
  3. Record inventory + `git format-patch` evidence under
     `.agent/evidence/c2-product-repo-install/`.
  4. When the Cloud Agent cannot push the product repo (403), ship the patch +
...
- **Consequences:** Product install is reproducible from control. Full C2 exit
  (merged product PR) may require Captain push or write access grant.
