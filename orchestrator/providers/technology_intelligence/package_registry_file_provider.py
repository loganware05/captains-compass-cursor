"""File-backed package-registry Technology Intelligence provider (offline fixtures)."""
...
DEFAULT_FIXTURES_DIR = Path(__file__).resolve().parent / "fixtures" / "package-registry"
_DEFAULT_TOP_N = 8


def _fixtures_dir() -> Path:
    override = os.environ.get("COMPASS_PACKAGE_TI_FIXTURES_DIR", "").strip()
    if override:
...
class PackageRegistryFileTechnologyIntelligenceProvider:
    """Load offline npm/PyPI-shaped package fixtures (no registry network)."""
        ...
        for path in sorted(self.fixtures_dir.glob("*.json")):
            ...
            for raw in records:
                if not isinstance(raw, dict):
                    continue
                candidates.append(candidate_from_package_registry_shaped(raw))
